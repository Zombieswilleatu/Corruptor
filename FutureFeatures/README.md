...
| Full-match rollback optimization | [Implemented, Windows comparison pending](../docs/U13_PYSIM_FULL_MATCH_COPYING_2026-09-15.md). Owned transactions share unchanged history/presentation; live-state access and custom handlers retain complete rollback. All 63 local tests/exact replay pass. The matched Linux CPython diagnostic measured 12.57 → 2.75 seconds (4.57×); the Windows runner compares 20 consecutive games per implementation/runtime under CPython and PyPy. |
...
1. Complete Windows acceptance and the sustained CPython/PyPy comparison for the implemented full-match rollback optimization. Retain `d059b95` as the control: CPython means 11.34 / 19.07 seconds and PyPy means 3.73 / 6.53. Preserve exact states/events, aliases, detached snapshots and failure rollback. Use the remaining measured cost for the next optimization, account for JIT warmup, and collect a matched Godot/Python pure-match comparison before extending the port further.
