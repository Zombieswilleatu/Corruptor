...
| Python mirror | Foundation through ordinary resolution retains its dated Windows acceptance. A separate `FullMatch` connects all 20 hooks and repeated rounds for Gremory, Deimos, Humbaba and Kalligan. The [ordinary full-match gate](../docs/U13_PYSIM_FULL_MATCH_2026-09-15.md) passed Windows 4.7.2 at clean `d059b95`: two complete games / 30 rounds / 767 operations, eight settlement components and one 200-tick probe. The [paid Development gate](../docs/U13_PYSIM_PAID_DEVELOPMENT_2026-09-16.md) passed Windows 4.7.2 at clean `24792a6`: four exact games / 57 rounds / 1,462 game operations, 258 paid component operations, 77 tests per Python runtime and 20 corruption rejections. CPython/PyPy reports match; all 17,904 native checks passed. Declared powers and five Lord integrations remain unsupported. |
...
2. Paid Rites/Resummon passed at `24792a6`. Extend persistent effects, declared powers and the remaining Lords, including timing, cooldowns, allegiance and spatial actors. Grow to approximately 50–100 exact full reference games after their mechanics are supported; the first two are not complete roster parity.
...
- **2026-09-16 start:** [Diagnostic checkpoint and revised sequence](../docs/U13_DOCTRINE_START_2026-09-16.md). Build fresh common doctrine with basic competence for all nine Lords and separate Lord files. U12 supplies selected ideas and regression cases, not the new architecture. Counters precede the planner; missing full-roster rules parity precedes shared-weight tuning. The Windows/PyPy diagnostics passed at `a7544d6`; paid Rites/Resummon parity subsequently passed at `24792a6`.
...
The Marching comparison is complete at `ad30730` with correctness accepted and mixed timing results. Doctrine diagnostics passed at `a7544d6`, and paid Rites/Resummon passed Windows dual-runtime parity at `24792a6`. Remaining declared powers and Lord integrations still precede shared planner tuning. The first complete-game reference, dual-runtime copying comparison and unchanged-engine profile remain accepted at `d059b95`, `c228d85` and `76e80fd` respectively. The next major design/content milestone is the monster system; the next presentation milestone is the UI overhaul. Campaign, prologue completion and optional online scope build on those foundations.
