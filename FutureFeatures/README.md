...
1. Profile the accepted optimized `c228d85` full-match implementation to locate its remaining cost, then measure the next change on identical inputs and hardware. The sustained Windows controls measured CPython 12.92 → 2.71 seconds and PyPy 7.83 → 1.45; retain all initial samples and the separate final-ten PyPy mean of 1.23 seconds. Preserve exact states/events, aliases, detached snapshots and failure rollback under both runtimes. The pre-change copying percentages do not identify the new dominant cost. Collect a matched Godot/Python pure-match comparison before extending the port further.
