...
1. Optimize transaction and retained event-history copying against the accepted `d059b95` full-game reference. Retain both Windows runtime baselines: CPython means 11.34 / 19.07 seconds and PyPy means 3.73 / 6.53. Copying dominates their separate profiles. Preserve exact states/events, aliases, detached public snapshots and failure rollback. Measure code changes on identical inputs/hardware under both runtimes, account for JIT warmup, and collect a matched Godot/Python pure-match comparison before extending the port further.
