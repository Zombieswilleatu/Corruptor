...
1. Optimize transaction and retained event-history copying against the accepted `d059b95` full-game reference. Windows means are 11.34 / 19.07 seconds; copying accounts for 76–78% of the separate profiles. Preserve exact states/events, aliases, detached public snapshots and failure rollback. Evaluate PyPy separately on the unchanged baseline with exact replay and JIT warmup. Measure code changes on identical inputs/hardware and collect a matched Godot/Python pure-match comparison before extending the port further.
