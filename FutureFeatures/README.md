...
1. Use the [focused profile runner](../docs/U13_PYSIM_OPTIMIZED_PROFILE_2026-09-15.md) on the accepted optimized `c228d85` full-match implementation to locate its remaining cost, then measure the next change on identical inputs and hardware. The sustained Windows controls measured CPython 12.92 → 2.71 seconds and PyPy 7.83 → 1.45; retain all initial samples and the separate final-ten PyPy mean of 1.23 seconds. Preserve exact states/events, aliases, detached snapshots and failure rollback under both runtimes. The local profile points to Marching and reaction-registry rebuilding; Windows/PyPy capture is pending. Collect a matched Godot/Python pure-match comparison before extending the port further.
...
The next concrete engineering milestone is profiling and improving the remaining cost in the accepted optimized full-match path, followed by the scope and throughput gates in section 2. The first complete-game reference and the dual-runtime copying comparison are accepted at `d059b95` and `c228d85` respectively. The next major design/content milestone is the monster system; the next presentation milestone is the UI overhaul. Campaign, prologue completion and optional online scope build on those foundations.
