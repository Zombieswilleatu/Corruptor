...
| Python Marching spike | Windows 4.7.2 acceptance passed at clean `e8cc3f9`: 28 phases / 5,600 tick frames, 394 Godot checks, 46 Python tests and 14 corruption probes. Independent replay reproduced the complete summary. Flat columns, explicit keyed contact ordering and prepared Web/aura/Rout/Gravity behavior are covered. The full-game adapter still stops after nine hooks. |
...
1. Integrate the accepted isolated Marching kernel with Post-Resolution and the remaining round lifecycle. Complete persistent effects and Lord mechanics, including timing, cooldowns, allegiance, pair breakage, Supplicants, reconstruction and deterministic retargeting; finish cleanup and victory.
2. Preserve the first legitimate independent setup-to-victory reference as soon as its mechanics are supported, then grow to approximately 50–100 exact full reference games. Do not wait for every Lord effect before retaining that first complete reference.
3. Measure complete matches on target hardware, separating rule execution, policy evaluation, event/export costs and worker scaling. Set the practical sweep budget from those results.
...
The accepted Marching spike uses parallel lists and separately measures validated import/publication against row dictionaries; it does not claim a row-versus-column tick-loop speedup. Windows single-worker means per 200-tick phase were 57.73 ms ordinary, 130.52 ms dense and 6.37 ms Gravity (13 of 14 units consumed). Dense median/p95 were 96.78 / 210.33 ms; the report records substantial variability. Movement/nearby searches dominate the separate instrumented profile. Explicit contact order, earliest arrival and keyed ties match, including reversed registry order. These phase timings establish no 50 ms whole-match budget or measured full-game speed. The earlier Linux diagnostic and Development-only timings retain their separate scopes.
