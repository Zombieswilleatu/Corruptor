> **2026-09-19 Rout / Humbaba implemented:** [Common doctrine V11 and the immediate Breath pulse](../docs/U13_ROUT_HUMBABA_2026-09-19.md), implementation `197f0db`, preserve the parallel Sinodek fix. Breath heals 1 HP immediately, then keeps its normal regeneration, movement bonus, duration and cooldown. Bots evaluate public Rout pressure and coordinate Breath with Muster/recruitment within the existing 32-plan / eight-preview limits. Local checks passed: 164 CPython tests; five games / 87 rounds / 2,187 operations, zero failures/rejections; 456 exact native/Python Breath transitions on Linux Godot 4.5.1. The 32-game baseline and controlled power-only replays are recorded. Next: Windows 4.7.2 + CPython/PyPy acceptance using `--humbaba-godot`, then a matched comparison on the same new rules before tuning or balance claims.
