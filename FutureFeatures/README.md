...
1. Use the [completed Windows profile](../docs/U13_PYSIM_OPTIMIZED_PROFILE_2026-09-15.md) to bound the next performance experiment: avoid unnecessary registry reconstruction after volleys without death reactions, then investigate movement work. Normalization overlaps reconstruction, and generic deep copies chiefly copy clocks/logs outside the tick loop. Another quick 2–3× is not established. Measure any candidate against the accepted `c228d85` source on identical inputs and hardware, retaining initial samples and separate later-game means. CPython varied across unchanged-source captures; use matched controls within each runtime. Preserve exact states/events, aliases, detached snapshots and failure rollback under both runtimes. Collect a matched Godot/Python pure-match comparison before extending the port further.
...
The next concrete engineering milestone is a bounded improvement experiment using the completed Windows profile, followed by the scope and throughput gates in section 2. The first complete-game reference, dual-runtime copying comparison and unchanged-engine profile are accepted at `d059b95`, `c228d85` and `76e80fd` respectively. The next major design/content milestone is the monster system; the next presentation milestone is the UI overhaul. Campaign, prologue completion and optional online scope build on those foundations.
