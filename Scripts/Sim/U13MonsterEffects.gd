...
static func step(world: Dictionary, entities, context: Dictionary, tick: int, reaction: Callable) -> Dictionary:
	...
	if tick == int(context.get("marching_tick_offset", 0)):
		var first_phase: bool = int(state.phase_round) < n
		state.phase_round = n
		...
		for unit in entities.marchers():
			...
			elif a.get("poison_until_round", 0) < n and a.has("poison_until_round"):
