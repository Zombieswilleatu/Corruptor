...
static func snapshot_game(
	...
	return {
		...
		"breach": _nullable_string(
			game_state.breach
		),
		"breach_owner": int(
			game_state.breach_owner
		),
		...
		"veil_total": int(
			game_state.calculate_veil_total()
		),
		"winner": _nullable_player_id(
			game_state.winner
		),
		"win_by": str(
			game_state.win_by
...
static func snapshot_player(
	...
	return {
		...
		"first_summon_done": bool(
			player.first_summon_done
		),
		"cataclysmic_used": bool(
			player.cataclysmic_used
		),
		"vessel_used": bool(
			player.vessel_used
		),
		"vessel_offered_lord": str(
			player.vessel_offered_lord
		),
		"kalligan_repair_used": bool(
			player.kalligan_repair_used
		),
		"kroni_ravenous_used": bool(
			player.kroni_ravenous_used
		),
		"deimos_breach_claimed": bool(
		...
		"garrison": card_multiset(
			player.garrison
		),
		"castle_guards": card_multiset(
			player.castle_guards
		),
		"lord_guards": card_multiset(
			player.lord_guards
		),
		"committed": card_multiset(
			player.committed
		),
...
static func card_multiset(
	cards: Array
) -> Array:
	var result: Array = card_list(
		cards
	)
...
static func sorted_strings(
	values: Array
) -> Array:
	var result: Array = string_list(
		values
	)
...
static func _canonicalize_json_value(
	value
):
	match typeof(
		value
...
static func _nullable_player_id(
	value
):
	if value == null:
		return null
