...
func _run_suite() -> void:
	...
	_add(world, "guard", 1, "Penitent", 1080, {"step_fp": 0})
	result = _run(world)
	var clashes: Array = _facts(result, "MARCHER_MELEE_ATTACK").filter(func(f): return f.attacker.attributes.suit == "Vulture")
	_check(not clashes.is_empty() and _facts(result, "MARCHER_RANGED_ATTACK").is_empty(), "Vulture still fights in melee")
	_check(not clashes.is_empty() and clashes[0].hp_after == 5 and clashes[0].damage_dealt == 0, "melee also deals one damage to armor")
	world = _world()
