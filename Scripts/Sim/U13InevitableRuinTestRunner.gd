...
func run() -> void:
	...
	for health in [0, 7, 13, 14, 15, 20, 21]:
		...
		for phase in ["declaration", "firing"]:
			check(content.validate(source, probe, phase).legal == (health > 14), "Ruin threshold %d at %s" % [health, phase])
		if health > 14:
			...
			check(castle.attributes.integrity == 14 and Work.Structures.operational(castle), "Ruin preserves an operational Castle at 14")
			...
			check(damage.data.damage == health - 14 and not damage.data.destroyed and result.world.data.neutral_tears == w.data.neutral_tears, "Ruin emits damage without destruction rewards")
			check(Ledger.describe(damage.type, damage.data).contains("Castle health 14"), "Aftermath states the remaining health")
		else:
	...
	for row in state.world.entities.entities:
		if row.kind == "castle" and row.owner == 1:
			row.attributes.integrity = 14 if row.attributes.castle_slot == 0 else row.attributes.integrity
	...
	check(Slots.castle_id(1, 0) not in options and Slots.castle_id(1, 1) in options, "Target list excludes 14 and includes a full-health Castle")
	check(board.ruin_state.text.contains("above 14") and board.ruin_state.text.contains("Reduces it to 14"), "Modal explains the threshold and result")
	board._intent = Gremory.RUIN; board._power_cost = board._available_ids().slice(0, 2)
	check(not board._target_allowed({"kind": "castle", "owner": 1, "id": Slots.castle_id(1, 0)}, Gremory.RUIN), "Board refuses the 14-health target")
