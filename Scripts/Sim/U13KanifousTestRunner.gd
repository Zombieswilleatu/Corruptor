			...
			for full in [false, true]:
				for pid in [0, 1]:
					...
					if expected:
						...
						check(restored.attributes.integrity == mini(8, int(restored.attributes.max_integrity)) and restored.attributes.status == "standing" and result.world.data.kanifous_prices.size() == 1, "Longevity repairs and charges " + label)
					else:
