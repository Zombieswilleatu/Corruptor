...
func run() -> void:
	...
	for pid in [0, 1]:
		...
		check(shots.size() <= 7 and shots[0].damage_dealt == 1, "tower uses one damage and the Vulture's 32-tick cadence")
		...
		clear_units(w)
		var enemy: Dictionary = put(w, "Butcher", 1 - pid, 900 if pid == 0 else 1500, {"y_fp": 150, "attack": 1, "hp": 100, "max_hp": 100}, 1)
