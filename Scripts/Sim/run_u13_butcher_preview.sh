...
u13_sheet=${2:-"$u13_root/ConceptImages/Sprites/ButcherSprite.png"}
if [[ ! -f "$u13_sheet" ]]; then
  echo "Sprite not found: $u13_sheet" >&2
  echo 'Pass the ButcherSprite.png path as the second argument.' >&2
  exit 1
fi
...
exec "$u13_godot" --path "$u13_root" --resolution 1100x800 \
  res://Prototype/U13/U13ButcherLanePreview.tscn -- "--butcher-sheet=$u13_sheet"
