# Test-only movement/combat reference frozen from cc0939d. Do not optimize this oracle.
# Killing-damage event metadata added for the Odradek Interlock contract.
extends RefCounted
...
static func resolve(context: Dictionary, reaction: Callable) -> Dictionary:
	...
	for tick in range(TICKS):
		...
		for lane in LANES:
			...
			var damage_to_left: int = _attack(left.attributes, int(right.attributes.attack), right.attributes.armor_bypass)
			var damage_to_right: int = _attack(right.attributes, int(left.attributes.attack), left.attributes.armor_bypass)
...
static func _attack(target: Dictionary, amount: int, bypass: bool) -> int:
	...
	# Keep the public killing-damage fact contract aligned with the live engine.
	return remaining
