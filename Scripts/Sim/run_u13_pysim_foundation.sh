#!/usr/bin/env bash
# Focused Windows 4.7.2 gate: exact transport, explicit Godot replay, Python opening parity.
set -euo pipefail
...
u13_py_reports=$(mktemp -d "$HOME/Downloads/u13-pysim-foundation-XXXXXX")
u13_py_pid=""
cleanup() {
  local result=$?
...
printf 'U13 PySim foundation reports: %s\n' "$u13_py_reports"
run_logged "$u13_py_reports/python-tests.log" "${u13_py_command[@]}" "$u13_py_root/Scripts/Sim/run_u13_pysim.py" self-test
run_logged "$u13_py_reports/godot.log" "$u13_py_exe" --headless --path "$u13_py_root" \
  --script Scripts/Sim/U13PySimFoundationTestRunner.gd -- \
  "$u13_py_reports/foundation.exact.json" "$u13_py_revision" "$u13_py_source"
grep -Eq '^U13 PySim foundation Godot failures: 0[[:space:]]*$' "$u13_py_reports/godot.log"
run_logged "$u13_py_reports/python-parity.log" "${u13_py_command[@]}" "$u13_py_root/Scripts/Sim/run_u13_pysim.py" \
  verify "$u13_py_reports/foundation.exact.json" --report "$u13_py_reports/python-summary.json"
grep -Eq '^U13 PySim foundation Python failures: 0[[:space:]]*$' "$u13_py_reports/python-parity.log"
printf 'U13 PySim foundation passed: exact opening parity and Godot trace replay.\n'
