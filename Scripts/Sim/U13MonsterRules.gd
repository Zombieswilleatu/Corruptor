...
const VERSION: String = "U13_MONSTERS_V5_STALKING_DOTRA"
...
const ROSTER: Dictionary = {
	...
	"Kopita": {"tier": "Moderate", "recipe": {"Wright": 2, "Penitent": 2}, "attack": 2, "armor": 1, "speed": 2, "hp": 5, "ability": "Pulses at the start of each active round, alternating green healing (allies recover 1 HP) and violet harm (enemies take 1 damage). Starts with healing; radius 360."},
	...
	"Dotra": {"tier": "Hard", "recipe": {"Butcher": 3, "Vulture": 1}, "attack": 2, "armor": 2, "speed": 2, "hp": 5, "ability": "25% chance to hide each active round. Stalks enemies at half speed while hidden; an enemy within 240 triggers a 5-damage ambush. Otherwise, chance to emerge starts at 15% next round, increasing by 15 percentage points each round hidden, capped at 100%. Resets after emerging. Hidden units cannot be selected for ordinary attacks."},
...
static func emerge_chance(a: Dictionary) -> int:
	return mini(100, 15 * (int(a.get("dotra_hidden_rounds", 0)) + 1))
