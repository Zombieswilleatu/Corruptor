...
const VERSION: String = "U13_MONSTERS_V14_DOTRA_BURROWS"
...
const TUNING: Dictionary = {"tumler_evasion_chance": 50, "tumler_hunt_bonus": 1, "kurchin_deflection_chance": 75, "varn_poison_chance": 10, "fyra_charm_chance": 30, "kopita_radius": 360, "kopita_second_pulse_tick": 133, "taunt_radius": 360, "muno_radius": 480, "dotra_hide_chance": 25, "dotra_ambush_radius": 240, "dotra_burrow_ticks": 24, "dotra_isolation_radius": 400, "sooge_root_chance": 25, "sooge_root_increase": 15, "beam_range": 1800, "beam_half_width": 70, "beam_interval_ticks": 200, "beam_charge_ticks": 32, "beam_blast_delay_ticks": 8, "sinodek_portal_chance": 25, "portal_target_range": 600, "portal_radius": 100, "portal_fear_radius": 300, "pool_radius": 200}
...
static func valid_unit(a: Dictionary) -> bool:
	...
	if a.has("dotra_ambush_ready") and typeof(a.dotra_ambush_ready) != TYPE_BOOL: return false
	if a.has("dotra_ambush_target") and typeof(a.dotra_ambush_target) != TYPE_STRING: return false
	if a.has("dotra_holes"):
		if typeof(a.dotra_holes) != TYPE_ARRAY or a.dotra_holes.size() not in [0, 3]: return false
		for hole in a.dotra_holes:
			if typeof(hole) != TYPE_DICTIONARY or not Data.is_integer(hole.get("x_fp")) or not Data.is_integer(hole.get("y_fp")): return false
			if hole.x_fp < 30 or hole.x_fp > 2370 or hole.y_fp < 30 or hole.y_fp > 570: return false
