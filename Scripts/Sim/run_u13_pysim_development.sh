#!/usr/bin/env bash
# Focused Windows 4.7.2 gate: Guard deployment, Work and stable pair parity; separate partial timing.
set -euo pipefail
...
u13_py_reports=$(mktemp -d "$HOME/Downloads/u13-pysim-development-XXXXXX")
u13_py_pid=""
cleanup() {
  local result=$?
...
printf 'U13 PySim development reports: %s\n' "$u13_py_reports"
run_logged "$u13_py_reports/python-tests.log" "${u13_py_command[@]}" "$u13_py_root/Scripts/Sim/run_u13_pysim.py" self-test-development
run_logged "$u13_py_reports/godot.log" "$u13_py_exe" --headless --path "$u13_py_root" \
  --script Scripts/Sim/U13PySimDevelopmentTestRunner.gd -- \
  "$u13_py_reports/development.exact.json" "$u13_py_revision" "$u13_py_source"
grep -Eq '^U13 PySim development Godot failures: 0[[:space:]]*$' "$u13_py_reports/godot.log"
run_logged "$u13_py_reports/python-parity.log" "${u13_py_command[@]}" "$u13_py_root/Scripts/Sim/run_u13_pysim.py" \
  verify-development "$u13_py_reports/development.exact.json" --report "$u13_py_reports/python-summary.json"
grep -Eq '^U13 PySim development Python failures: 0[[:space:]]*$' "$u13_py_reports/python-parity.log"
run_logged "$u13_py_reports/python-timing.log" "${u13_py_command[@]}" "$u13_py_root/Scripts/Sim/run_u13_pysim.py" \
  benchmark-development --report "$u13_py_reports/partial-timing.json"
printf 'U13 PySim development passed: six hooks and directed Guard/Work lifecycles. Full-match speed remains unknown.\n'
