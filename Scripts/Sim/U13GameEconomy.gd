...
const VERSION: String = "U13_GAME_ECONOMY_V5_FREE_OPENING"
# Keep the tested physical deck/trim sequence when versioning opening rules.
const SHUFFLE_VERSION: String = "U13_GAME_ECONOMY_V4"
...
static func initialize(raw: Dictionary, seed_value: String) -> Dictionary:
	...
	# Lords start free. The first hand comes from the ordinary round-one draw,
	# including the usual Stockpile/Slaver choices; there is no extra setup deal.
	var summons: Dictionary = _free_opening_summons(world)
...
# Free first summons still have an exact ledger and count as summon one.
# Later Resummon payments, Blood Offering and Fracture are separate rules.
static func _free_opening_summons(world: Dictionary) -> Dictionary:
	...
	for pid in [0, 1]:
		...
		if actor.is_empty() or actor.owner != pid or not Resummon.COSTS.has(actor.attributes.get("lord_id")):
		...
		records.append({
			"player_id": pid, "lord_id": actor.attributes.lord_id,
			"lord_entity_id": actor.id, "cost": 0, "paid_value": 0,
			"shortfall": 0, "card_ids": [], "card_values": [],
...
static func _shuffle(cards: Array, seed_value: String, purpose: String) -> void:
	for index in range(cards.size() - 1, 0, -1):
		var pick: int = int(Rng.draw(seed_value, SHUFFLE_VERSION, purpose, index, index + 1).value)
...
static func _opening_valid(world: Dictionary, opening) -> bool:
	...
	for pid in [0, 1]:
		...
		if (
			...
			return false
		if (
			record.cost != 0 or record.paid_value != 0 or record.shortfall != 0
			or not record.card_ids.is_empty() or not record.card_values.is_empty()
			or not record.circle_id.is_empty() or record.circle_exerted != 0
		):
			return false
	return true
