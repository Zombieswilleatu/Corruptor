#!/usr/bin/env bash
# Focused Windows 4.7.2 gate: isolated Marching parity and measured phase/layout costs.
set -euo pipefail
...
u13_py_reports=$(mktemp -d "$HOME/Downloads/u13-pysim-marching-XXXXXX")
u13_py_pid=""
cleanup() {
  local result=$?
...
run_logged() {
  ...
  # Preserve the bounded 420s export/replay watchdog used by the prior gate.
...
printf 'U13 PySim marching reports: %s\n' "$u13_py_reports"
run_logged "$u13_py_reports/python-tests.log" "${u13_py_command[@]}" "$u13_py_root/Scripts/Sim/run_u13_pysim.py" self-test-marching
run_logged "$u13_py_reports/godot.log" "$u13_py_exe" --headless --path "$u13_py_root" \
  --script Scripts/Sim/U13PySimMarchingTestRunner.gd -- \
  "$u13_py_reports/marching.exact.json" "$u13_py_revision" "$u13_py_source"
grep -Eq '^U13 PySim marching Godot failures: 0[[:space:]]*$' "$u13_py_reports/godot.log"
run_logged "$u13_py_reports/python-parity.log" "${u13_py_command[@]}" "$u13_py_root/Scripts/Sim/run_u13_pysim.py" \
  verify-marching "$u13_py_reports/marching.exact.json" --report "$u13_py_reports/python-summary.json"
grep -Eq '^U13 PySim marching Python failures: 0[[:space:]]*$' "$u13_py_reports/python-parity.log"
run_logged "$u13_py_reports/python-timing.log" "${u13_py_command[@]}" "$u13_py_root/Scripts/Sim/run_u13_pysim.py" \
  benchmark-marching --verified-report "$u13_py_reports/python-summary.json" --report "$u13_py_reports/marching-timing.json"
grep -Eq '^U13 PySim marching timing failures: 0[[:space:]]*$' "$u13_py_reports/python-timing.log"
printf 'U13 PySim marching spike passed: isolated 200-tick phases and layout/timing measurements. Full-match speed remains unknown.\n'
