	...
	match action:
		...
		"hunger":
			...
			if not hunger_state.active(world, pid):
				return Data.invalid("debug_requires_active_kroni")
			events.append_array(hunger_state.feed(world, pid, 1, round_number, "Debug"))
			message = "Kroni gains 1 Hunger; normal milestone rules apply."
		"reconfiguration":
