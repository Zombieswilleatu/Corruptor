...
func work_case() -> Dictionary:
	...
	settle(t, 1)
	check(row(t.current, target).attributes.integrity == 9, "fresh Guards plus Wright pair plus passive Work")
	stage(t, 2); settle(t, 2)
	component_record(t, {"kind": "work", "round": 2, "player_order": [0, 1]})
	check(row(t.current, target).attributes.integrity == 12, "survivors do not repeat Work or Wright bonus")
	stage(t, 3, [move(give(t, "Wright"), "Castle", 2), move(give(t, "Wright"), "Castle", 0)])
	settle(t, 3)
	check(row(t.current, target).attributes.integrity == 17 and t.current.data.guard_work.targets[0].is_empty(), "completion clamps, activates and clears target")
...
func reconstruction_case() -> Dictionary:
	...
	check(row(t.current, engine).attributes.integrity == 8 and not row(t.current, engine).attributes.has("repair_lock_until_round"), "Deimos reconstructs same Engine identity as protected building")
