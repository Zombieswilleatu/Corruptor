"""Compare focused native Scorch records with independently executed Python."""
import hashlib
import json
from pathlib import Path
from . import codec, power_components, scorch_castle_cases
from .power_match import PowerMatch
from .verify import same, source_identity


def export_inputs(path):
    Path(path).write_text(json.dumps(scorch_castle_cases.generate(),indent=2,sort_keys=True)+'\n',encoding='utf-8')
...
def verify(path,inputs_path,root):
    ...
    same(scorch_castle_cases.generate(),manifest,'current_directed_inputs')
    ...
    with path.open(encoding='utf-8') as stream:
        def take():
            ...
            if not line:raise ValueError('Truncated Scorch evidence')
            count+=1
            return codec.loads(line)
        header=take()
        same('U13_SCORCH_CASTLE_EXACT_V1',header['schema'],'schema')
        ...
        if stream.read().strip():raise ValueError('Unexpected extra Scorch evidence')
