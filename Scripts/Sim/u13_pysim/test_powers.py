    ...
    def test_inevitable_ruin_targets_only_castles_above_fourteen(self):
        for health in (0, 7, 13, 14, 15, 20, 21):
            with self.subTest(health=health):
                ...
                castle['attributes'].update(integrity=health,status='standing' if health else 'defunct')
        ...
        for health in (0, 13, 14, 15, 21):
            with self.subTest(health=health):
                ...
                for entry in case['operations'][:-1]:
                    self.assertEqual(entry['rejected'],power_components.apply(g,entry['operation'])['action']=='invalid')
                w=g._state['world'];pending=g._state['pending']['pending'][0]
                castle=e.entity(w,pending['declaration']['target']['entity_id'])
                castle['attributes'].update(integrity=health,status='standing' if health else 'defunct')
