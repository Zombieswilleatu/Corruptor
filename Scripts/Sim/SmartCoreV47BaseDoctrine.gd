...
# U12_COMMIT_POLICY_DIVERGENCE_SIM_V1
# Dormant unless explicitly enabled by the experiment runner.
static var _u12_commit_policy_sim_enabled: bool = false
static var _u12_commit_policy_sim_rows: Array[Dictionary] = []


static func u12_commit_policy_sim_start() -> void:
	_u12_commit_policy_sim_rows.clear()
	_u12_commit_policy_sim_enabled = true


static func u12_commit_policy_sim_stop() -> Array[Dictionary]:
	_u12_commit_policy_sim_enabled = false
	var out: Array[Dictionary] = []
	for row in _u12_commit_policy_sim_rows:
		out.append(row.duplicate(true))
	return out


static func _u12_commit_policy_sim_cards(
	decision: Dictionary
) -> Array[String]:
	var result: Array[String] = []
	var raw_cards = decision.get("cards", [])
	if typeof(raw_cards) == TYPE_ARRAY:
		for raw_card in raw_cards:
			result.append(str(raw_card))
	result.sort()
	return result


static func _u12_commit_policy_sim_target(
	decision: Dictionary
) -> String:
	return "%s|%s|%s" % [
		str(decision.get("target_pid", "")),
		str(decision.get("target_type", "")),
		str(decision.get("target_castle", "")),
	]


static func _u12_commit_policy_sim_top_two(
	candidates: Array
) -> Dictionary:
	var top_score: float = -1.0e300
	var second_score: float = -1.0e300
	var top_id: String = ""
...
static func _u12_commit_policy_sim_record(
	game,
	player_id: int,
	candidates: Array,
	actual_selection: Dictionary,
	actual_decision: Dictionary,
	rules: RuleConfig
) -> void:
	if not _u12_commit_policy_sim_enabled:
		return
	...
	var golden_candidate: Dictionary = (
		golden_selection.get("candidate", {})
	)
	var golden_decision: Dictionary = (
	...
	var actual_action: String = str(
		actual_decision.get("action", "")
	)
	var golden_action: String = str(
		golden_decision.get("action", "")
	)
	var actual_target: String = (
		_u12_commit_policy_sim_target(actual_decision)
	)
	var golden_target: String = (
		_u12_commit_policy_sim_target(golden_decision)
	)
	var actual_cards: Array[String] = (
		_u12_commit_policy_sim_cards(actual_decision)
	)
	var golden_cards: Array[String] = (
		_u12_commit_policy_sim_cards(golden_decision)
	)

	var action_diff: bool = actual_action != golden_action
	var target_diff: bool = actual_target != golden_target
	var cards_diff: bool = actual_cards != golden_cards
	var divergence: bool = action_diff or target_diff or cards_diff

	var score_info: Dictionary = (
		_u12_commit_policy_sim_top_two(candidates)
	)

	var actual_index: int = int(
		actual_selection.get("index", -1)
	)
	var actual_probability: float = -1.0
	var raw_probabilities = actual_selection.get("probabilities", [])

	if (
		typeof(raw_probabilities) == TYPE_ARRAY
		and actual_index >= 0
		and actual_index < raw_probabilities.size()
	):
		actual_probability = float(
			raw_probabilities[actual_index]
		)

	var argmax_index: int = int(
		golden_selection.get("index", -1)
	)
	var argmax_probability: float = -1.0

	if (
		typeof(raw_probabilities) == TYPE_ARRAY
		and argmax_index >= 0
		and argmax_index < raw_probabilities.size()
	):
		argmax_probability = float(
			raw_probabilities[argmax_index]
		)

	var player = game.get_player(player_id)
	var opponent = game.get_opponent(player_id)

	_u12_commit_policy_sim_rows.append({
		"round": int(game.round),
		"player_id": player_id,
		"lord": (
			str(player.lord)
		...
		"standard_candidate": str(
			actual_selection.get("candidate_id", "")
		),
		"golden_candidate": str(
			golden_selection.get("candidate_id", "")
		),
		"standard_action": actual_action,
		"golden_action": golden_action,
		"standard_target": actual_target,
		"golden_target": golden_target,
		"standard_cards": actual_cards,
		...
		"standard_roll": float(
			actual_selection.get("roll", -1.0)
		),
		"standard_probability": actual_probability,
		"argmax_probability": argmax_probability,
		"expected_non_argmax_probability": (
			-1.0
			if argmax_probability < 0.0
			else 1.0 - argmax_probability
		),
		"standard_was_argmax": (
			str(actual_selection.get("candidate_id", ""))
			== str(golden_selection.get("candidate_id", ""))
		),
		"top_candidate": str(
			score_info.get("top_candidate", "")
		),
		"second_candidate": str(
			score_info.get("second_candidate", "")
		),
		"top_score": float(
			score_info.get("top_score", 0.0)
		),
		"second_score": float(
			score_info.get("second_score", 0.0)
		),
		"top_two_gap": float(
			score_info.get("gap", -1.0)
		),
	})
...
static func commitment_choices(
	...
	for player in game.players:
		...
		decisions[player_id] = commitment_decision
		_culp_commit_mark(
