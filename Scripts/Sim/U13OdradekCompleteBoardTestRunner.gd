...
func _run() -> void:
	...
	board.reconfiguration_entry.pressed.emit()
	await _settle()
	_check(board.reconfiguration_menu.visible and not board.phase_prompt.visible, "reconfiguration_opens_dedicated_menu_without_overlapping_main_modal")
	_check(board.odradek_box.get_parent() is ScrollContainer, "power_options_scroll_inside_dedicated_menu")
	...
	await _settle()
	board.sides[1].castle_guard_box.get_child(0).input_surface.pressed.emit()
	_check(board.guard_destination.get("owner", -1) == 0 and "Destination: ENEMY" in board.guard_targeting.note.text, "self_inversion_selects_own_zone_and_names_enemy_destination")
	board.guard_targeting.confirm_button.pressed.emit()
	_check(board.queued.size() == 1 and board.queued[0].target.owner_id == 0, "self_inversion_queues_through_board")
	board.clear_powers()
	board.inversion_button.pressed.emit()
	await _settle()
	board.sides[0].castle_guard_box.get_child(0).input_surface.pressed.emit()
