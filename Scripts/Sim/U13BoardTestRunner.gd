...
func _init() -> void:
	# Headless has no display/vsync pacing. Let worker threads run between UI
	# frames instead of spinning the scene tree as fast as the CPU can manage.
...
func _run() -> void:
	...
	# Model/replay coverage runs in U13BoardModelTestRunner with its own budget.
	# Keep this process focused on real UI actions and asynchronous workers.
	_stage("board_controls", started)
	await _board_controls()
	_stage("complete", started)
...
func _wait_board_job(board) -> void:
	if board._job == null:
		return
	var started: int = Time.get_ticks_msec()
	var deadline: int = started + 15000
	var next_report: int = started + 2000
	var operation: String = board._job_operation
	print("BOARD WAIT begin operation=%s" % operation)
	while board._job != null and Time.get_ticks_msec() < deadline:
		await process_frame
		if not board.is_processing():
			board._process(0.0)
		if board._job != null and Time.get_ticks_msec() >= next_report:
			print(
				(
					"BOARD WAIT pending operation=%s elapsed_ms=%d"
					% [operation, Time.get_ticks_msec() - started]
				)
			)
			next_report = Time.get_ticks_msec() + 2000
	print(
		"BOARD WAIT end operation=%s elapsed_ms=%d" % [operation, Time.get_ticks_msec() - started]
	)
	if not _check(board._job == null, "board_worker_completed"):
		# Do not run more assertions or launch more jobs after a worker deadline.
		quit(1)
		await process_frame
...
func _stage(label: String, started: int) -> void:
	print("BOARD STAGE %s elapsed_ms=%d" % [label, Time.get_ticks_msec() - started])
