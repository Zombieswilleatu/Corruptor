...
static func march_choice(
	...
	var reactive_lane: String = MarchingEngineData.reactive_lane_for(
		game,
		player,
		rules
	)
	...
	var enemy_marcher = (
		_marcher_in_lane(opponent, reactive_lane)
		if not reactive_lane.is_empty()
		else _threatening_marcher(opponent, rules)
	)
	...
	return {
		"action": "march",
		"source_zone": source_zone,
		"lane": _choose_lane(card, source_zone, opponent, rules, reactive_lane),
		"card": _card_id(card),
	}
...
static func _marcher_in_lane(
	player,
	lane: String
):
	if player == null or lane.is_empty():
		return null
