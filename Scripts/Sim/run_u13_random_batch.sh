...
set -uo pipefail
if [[ $# -lt 1 || ! -x "$1" ]]; then
  printf 'Usage: bash %s /path/to/Godot.exe [--trials=4] [--rounds=6] [--seed-prefix=name] [--roster=gremory|deimos|mixed|construction|loadout|humbaba|kalligan]\n' "$0" >&2
  exit 2
fi
u13_batch_exe=$1
shift
...
for u13_batch_arg in "$@"; do
  case "$u13_batch_arg" in
    ...
    --roster=gremory|--roster=deimos|--roster=mixed|--roster=construction|--roster=loadout|--roster=humbaba|--roster=kalligan) u13_batch_roster=${u13_batch_arg#--roster=} ;;
