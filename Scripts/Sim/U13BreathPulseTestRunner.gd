...
func run() -> void:
	...
	if not check(inputs.get("schema") == "U13_BREATH_PULSE_CASES_V1", "Breath input schema"):
		quit(1); return
	var header: Dictionary = {"kind": "header", "schema": "U13_BREATH_PULSE_EXACT_V1",
	...
	if not check(output != null, "Breath output opens"): quit(1); return
	...
	check(replay_index == digests.size(), "all Breath records independently replayed")
	print("U13 Breath pulse Godot checks: %d" % checks)
	print("U13 Breath pulse Godot failures: %d" % failures)
