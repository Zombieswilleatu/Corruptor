...
const Reference = preload("res://Scripts/Sim/U13InstallNormalizationReference.gd")
var failures: int = 0
var checks: int = 0
var cases: Array = []
...
func run() -> void:
	...
	var checkpoints_only: bool = false
	for arg in OS.get_cmdline_user_args():
		if arg == "--checkpoints-only": checkpoints_only = true
			...
			return
	boundaries()
	for index in ([] if checkpoints_only else [9, 19, 29, 39, 49, 59, 69, 79, 8]):
	...
	var report: Dictionary = {"engine": Engine.get_version_info().string, "reference_commit": "e4d9501494664d3d5504e77a5d52ae02c51ee8d9", "checks": checks, "failures": failures, "totals": totals, "cases": cases}
	if not output.is_empty():
		var file = FileAccess.open(output, FileAccess.WRITE)
		if check(file != null, "profile report opened"):
			file.store_string(JSON.stringify(report, "\t"))
	print("Resolution totals (execution only): ", JSON.stringify(totals))
	print("U13 install normalization failures: %d" % failures)
	quit(failures)
...
func boundaries() -> void:
	...
	# Callback mutation must not change the installed canonical world or caller.
	for cache_enabled in [false, true]:
		var isolated = owner._fork_validated()
		var isolated_old = Reference.from_owner(owner)
		isolated._cache_world_validation = cache_enabled
		isolated_old._cache_world_validation = cache_enabled
		var mutator: Callable = func(value):
			value.players[0].resources.souls = -999
			value.data.clear()
			return true
		isolated._world_validator = mutator
		isolated_old._world_validator = mutator
		check(same(isolated._install_world(world), isolated_old._install_world(world)) and same(isolated.snapshot(), original) and same(isolated.snapshot(), isolated_old.snapshot()), "validator mutations stay isolated, cache=" + str(cache_enabled))
