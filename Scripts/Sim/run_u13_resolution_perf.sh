#!/usr/bin/env bash
# Compare the current resolution path with the frozen 217565b world installer.
set -euo pipefail
...
u13_perf_reports=${2:-$(mktemp -d "$HOME/Downloads/u13-resolution-${u13_perf_commit:0:8}-XXXXXX")}
...
printf 'Resolution reports: %s\nChecking unchanged-world validation and nine Lord pairs...\n' "$u13_perf_reports"
"$u13_perf_exe" --headless --path "$u13_perf_root" --script Scripts/Sim/U13ResolutionCacheTestRunner.gd -- "--output=$u13_perf_reports/resolution.json" >"$u13_perf_reports/resolution.log" 2>&1 &
u13_perf_pid=$!
...
while kill -0 "$u13_perf_pid" 2>/dev/null; do
  if ((SECONDS >= u13_perf_deadline)); then
    printf 'FAIL resolution performance watchdog (600 seconds). Reports: %s\n' "$u13_perf_reports" >&2
    exit 1
  fi
  if ((SECONDS >= u13_perf_heartbeat)); then
    printf 'Elapsed %sm %ss\n' "$((SECONDS / 60))" "$((SECONDS % 60))"
    tail -n 1 -- "$u13_perf_reports/resolution.log"
    u13_perf_heartbeat=$((SECONDS + 15))
  fi
  sleep 0.2
done
...
u13_perf_pid=""
if ((u13_perf_status != 0)) || grep -Eq 'SCRIPT ERROR|ERROR:|^FAIL ' "$u13_perf_reports/resolution.log" || ! grep -Eq '^U13 resolution cache failures: 0$' "$u13_perf_reports/resolution.log"; then
  tail -n 40 -- "$u13_perf_reports/resolution.log"
  printf 'FAIL resolution verification. Reports: %s\n' "$u13_perf_reports" >&2
  exit 1
fi
grep -E '^PROFILE |^Resolution totals|^U13 resolution cache failures:' "$u13_perf_reports/resolution.log"
printf 'Resolution checks passed. Reports: %s\n' "$u13_perf_reports"
