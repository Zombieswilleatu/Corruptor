...
func run() -> void:
	...
	check(board._order_preview.get_children().any(func(c): return c is Label and c.text.contains("Visible offense:") and c.text.contains("Visible defense:")), "compact forecast is under commitment stack")
