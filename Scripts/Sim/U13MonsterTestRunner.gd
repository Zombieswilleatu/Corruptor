...
func muno_dash_checks() -> void:
	...
	for pid in [0, 1]:
		...
		check(facts(resolved, "MONSTER_ATTACK").filter(func(d): return d.ability == "Muno").map(func(d): return d.tick) == [0, 100], "Muno makes two free strikes at the 100-tick cooldown")
		...
		check(retreat.monster_attacks[0].ability == "MunoDash" and returned.monster_attacks.size() == 1 and playback.sample(0.8).monster_attacks.any(func(a): return a.ability == "MunoAfterimage"), "retreat afterimages persist while Muno's damage absorber remains charged")
...
func kopita_pulse_checks() -> void:
	...
	for pid in [0, 1]:
		...
		check(hits.all(func(h): return h.target.id in [enemy.id, armored.id] and h.tick in [0, 133]) and hits.any(func(h): return h.target.id == enemy.id and h.damage_dealt == 2) and hits.any(func(h): return h.target.id == armored.id and h.damage_dealt == 0), "both harm pulses obey range, lane and Armor")
...
func run() -> void:
	...
	for name in ["Lemek", "Fyra", "Varn", "Kopita", "Tumler", "Kurchin", "Muno", "Dotra"]:
		...
		if name == "Muno": check(facts(result, "MONSTER_ATTACK").filter(func(d): return d.ability == "Muno").size() == 2, "Muno free strikes repeat on the shorter cooldown")
...
func special_checks() -> void:
	...
	world.entities = buffer.snapshot()
	var c: Dictionary = context(world)
	var poisoned: Dictionary = MonsterFX.step(world, buffer, c, 0, Callable(content, "react"))
	check(buffer.get_entity(victim.id).attributes.hp == 4 and buffer.get_entity(victim.id).attributes.armor == 2, "poison damages HP through armor immediately")
	MonsterFX.step(poisoned.world, buffer, c, 27, Callable(content, "react"))
	check(buffer.get_entity(victim.id).attributes.hp == 3, "poison ticks again after about two seconds")
	MonsterFX.step(poisoned.world, buffer, c, 54, Callable(content, "react"))
	MonsterFX.step(poisoned.world, buffer, c, 81, Callable(content, "react"))
	check(buffer.get_entity(victim.id).attributes.hp == 2 and buffer.get_entity(victim.id).attributes.poison_ticks_left == 0, "poison expires after exactly three ticks")
