...
func _reference_equivalence(count: int) -> void:
	for mode in ["travel", "contact"]:
		var case_started: int = Time.get_ticks_msec()
		print("SPATIAL CASE count=", count, " mode=", mode, " BEGIN")
		var ids = Ids.new()
		...
		for index in range(count):
			...
			var created: Dictionary = ids.create("marcher", "equivalence", index, owner, attributes)
		...
		for round_number in [1, 2]:
			...
			var reference_started: int = Time.get_ticks_msec()
			...
			var reference_ms: int = Time.get_ticks_msec() - reference_started
			print("SPATIAL optimized BEGIN reference_ms=", reference_ms)
			var optimized_started: int = Time.get_ticks_msec()
			var actual: Dictionary = _run(world, round_number)
			print(
				"SPATIAL TIMING count=",
				count,
				" mode=",
				mode,
				" round=",
				...
				reference_ms,
				" optimized_ms=",
				Time.get_ticks_msec() - optimized_started
			)
			if not _check(
		...
		print(
			...
			mode,
			" elapsed_ms=",
			Time.get_ticks_msec() - case_started
		)
