...
# High HP prevents deaths from hiding extra hits in the live 6/4 profile.
func run() -> void:
	var args: PackedStringArray = OS.get_cmdline_user_args()
	if not args.is_empty(): phase_output = FileAccess.open(args[0], FileAccess.WRITE)
	check(Melee.INTERVAL == 34 and Ranged.EXCHANGE_TICKS == 34 and Ranged.RANGED_INTERVAL_TICKS == 50, "live cadence is six melee / four ranged per round")
