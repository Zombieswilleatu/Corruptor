#!/usr/bin/env bash
# Focused Windows 4.7.2 gate: first complete ordinary games, settlement parity and actual match timing.
set -euo pipefail
...
u13_py_reports=$(mktemp -d "$HOME/Downloads/u13-pysim-full-match-XXXXXX")
u13_py_pid=""
cleanup() {
  local result=$?
...
run_logged() {
  ...
  "$@" >"$log" 2>&1 &
  u13_py_pid=$!
  # Two complete games, then independent native replay; full state at every hook.
  # This is a focused parity gate, not a 100-game campaign.
  local stage_limit=900
  if [[ "$log" == */godot.log ]]; then stage_limit=2400; fi
  ...
  tail -n 3 -- "$log"
  printf '%s=%s seconds\n' "$(basename -- "$log")" "$((SECONDS - started))" >>"$u13_py_reports/stage-times.txt"
}
printf 'U13 PySim full-match reports: %s\n' "$u13_py_reports"
run_logged "$u13_py_reports/python-tests.log" "${u13_py_command[@]}" "$u13_py_root/Scripts/Sim/run_u13_pysim.py" self-test-full-match
run_logged "$u13_py_reports/godot.log" "$u13_py_exe" --headless --path "$u13_py_root" \
  --script Scripts/Sim/U13PySimFullMatchTestRunner.gd -- \
  "$u13_py_reports/full-match.exact.jsonl" "$u13_py_revision" "$u13_py_source"
grep -Eq '^U13 PySim full-match Godot failures: 0[[:space:]]*$' "$u13_py_reports/godot.log"
run_logged "$u13_py_reports/python-parity.log" "${u13_py_command[@]}" "$u13_py_root/Scripts/Sim/run_u13_pysim.py" \
  verify-full-match "$u13_py_reports/full-match.exact.jsonl" --report "$u13_py_reports/python-summary.json"
grep -Eq '^U13 PySim full-match Python failures: 0[[:space:]]*$' "$u13_py_reports/python-parity.log"
run_logged "$u13_py_reports/python-timing.log" "${u13_py_command[@]}" "$u13_py_root/Scripts/Sim/run_u13_pysim.py" \
  benchmark-full-match --verified-report "$u13_py_reports/python-summary.json" --report "$u13_py_reports/full-match-timing.json"
grep -Eq '^U13 PySim full-match timing failures: 0[[:space:]]*$' "$u13_py_reports/python-timing.log"
printf 'U13 PySim full-match gate passed: two complete ordinary games, exact states/events, settlement boundaries and measured match timing. Other Lords/powers remain outside this gate.\n'
