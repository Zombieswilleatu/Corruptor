#!/usr/bin/env python3
"""Install a temporary phase-result probe for the current soak divergence."""
...
MARKER = "DEBUG SOAK GREMORY KANIFOUS R7"


def patch(text: str) -> str:
    if MARKER in text:
        raise RuntimeError("REFUSED: round-7 phase probe is already installed")
    ...
    probe = (
        f'{indent}if (\n'
        f'{deep}int(game.round) == 7\n'
        f'{deep}and game.players.size() == 2\n'
        f'{deep}and String(game.players[0].lord) == "Gremory"\n'
        f'{deep}and String(game.players[1].lord) == "Kanifous"\n'
        f'{deep}and int(game.winner) >= 0\n'
        f'{deep}and String(game.win_by) == "FinalCollapse"\n'
        f'{indent}):\n'
        f'{deep}print(\n'
...
def main() -> int:
    ...
    print("Installed temporary Gremory-vs-Kanifous round-7 phase probe.")
