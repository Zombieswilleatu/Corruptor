...
func _build_marching_board() -> Control:
	...
	var legend := _new_label(
		"YOUR GUARDS →  step 1  ·  step 2  ·  enemy gate (step 3)    •    OPPONENT GUARDS mirror this track ←",
		12
	)
...
func _refresh_marching_board() -> void:
	...
	if march_status_label != null:
		if controller.stage == PlayableRoundControllerData.Stage.MARCH:
			march_status_label.text = "1. Choose Guard zone   2. Select one Guard   3. Choose lane   4. Launch or pass"
	...
	if controller != null and controller.game != null:
		...
		if controller.stage == PlayableRoundControllerData.Stage.MARCH:
			target_info_label.text = (
				...
				+ "If both players march in the same lane, a lane clash deals %d to each marcher; a suit advantage adds +%d. "
...
func _march_picker_prompt() -> String:
	...
	if human.marchers.size() >= controller.rules.march_max_in_flight:
		return "You already have a marcher in flight. Pass this step."
	...
	return (
		...
		+ "3. Choose a lane.  4. Launch it face-up. "
		+ "It advances after Resolution; value %d+ at the enemy gate gains a Tear."
	) % [source_zone, controller.rules.march_threshold]
