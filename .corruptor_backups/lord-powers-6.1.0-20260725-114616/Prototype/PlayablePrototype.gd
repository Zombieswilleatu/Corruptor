...
const LORD_CARD_ABILITIES: Dictionary = {
	"Orias": [
		...
		"[b]Breach: Frenzy[/b] — Players at Threat 3+ cannot Deploy to Garrison.",
	],
	"Deimos": [
	...
	"Valak": [
		...
		"[b]Breach: Waning[/b] — At Resolution Prelude, each previously attacked zone loses its lowest Guard.",
	],
	"Kroni": [
		"[b]Consume / Gorge[/b] — Once per round after any destruction, gain 1 Hunger. If Kroni personally defeated a Guard, also gain 1 Soul.",
		...
		"[b]Fallback / Decay / Breach[/b] — If Consume did not fire, sacrifice the lowest Guard or Garrison card. Ward/Pass loses 1 Hunger. Kroni Breach destroys each player's lowest Guard.",
	],
	"Kalligan": [
		...
		"[b]Inferno / Wildfire[/b] — After ruining a Castle, Kalligan may gain 1 Threat to discard the highest enemy Lord Guard; the next Prelude Scorches the enemy Castle zone (or Lord zone if none remain), discarding every value-1/2 Guard there.",
	...
	"Gremory": [
		...
		"[b]Ruinous Harvest[/b] — Once per round when a Tear is created, recover the newest value-4+ card from the discard. The first Ruin each round also recovers the newest discard.",
		"[b]Grave Insight[/b] — The first time a Lord Guard is defeated each round, draw 1 outside Development, then discard the lowest card in Hand.",
	...
	"Kanifous": [
		"[b]Invoke[/b] — At Reveal, gain 1 Threat, reveal 2 cards, choose one to bank in Garrison, and discard the other. If the first revealed card is value 4+, also create a Neutral Tear.",
		"[b]Suit Invocation[/b] — Vulture: draw 3 then discard the lowest Hand card. Wright: move up to 2 Lord Guards to Castle. Penitent: draw 2 temporary Guards. Butcher: ignore the lowest target Guard this round.",
		...
		"[b]Breach[/b] — Every draw outside Development gives its recipient +1 Threat.",
	],
	"Humbaba": [
...
func _build_footer_row() -> Control:
	...
	var note := _new_label(
		"Development is automated in this first vertical slice.",
		15
	)
	note.modulate = Color(
		0.75,
...
func _on_resolve_pressed() -> void:
	var before: Dictionary = _state_digest()

	var result: Dictionary = (
		controller.resolve_revealed_round()
	)
	...
	_log_guard_reveals()
	_log_resolution_transcript(
		result
	)

	_log_resolution_changes(
		before,
		_state_digest()
	)
...
func _log_prelude_lord_powers(
	...
	for raw_event in _array_from(
		...
		_log(
			"[color=#d8b4fe][b]Valak Breach — Waning:[/b] %s lost %s from the previously attacked %s zone.[/color]"
			% [
				_player_name(
					int(
						event.get(
...
func _log_aftermath_lord_powers(
	...
	if not vulture_draw.is_empty():
		_log(
			"[color=#d8b4fe][b]Vulture pair:[/b] %s drew %s outside Development.[/color]"
			% [
				_player_name(
					player_id
				),
...
func _log_finale_lord_powers(
	...
	for raw_event in _array_from(
		...
		var event: Dictionary = raw_event
		_log(
			"[color=#d8b4fe][b]Kroni — Fallback Consume:[/b] Sacrificed %s from %s; Hunger %d→%d.[/color]"
			% [
				String(
					event.get(
						"discarded_card",
...
func _log_gremory_guard_trigger(
	...
	_log(
		"[color=#d8b4fe][b]Gremory — Grave Insight:[/b] %s triggered it for %s; drew %s and discarded lowest %s.[/color]"
		% [
			source_name,
			_player_name(
				player_id
...
func _kanifous_invoke_effect_text(
	...
	match suit:
		"Vulture":
			return (
				"drew %s outside Development, discarded %s, then %s"
				% [
					(
						"%d hidden card(s)"
						% _array_from(
...
func _log_draw_side_effects(
	...
	for raw_result in _array_from(
		...
		if bool(
			...
			_log(
				"[color=#d8b4fe][b]Kanifous Breach:[/b] %s's outside-Development draw raised Threat %d→%d during %s.[/color]"
				% [
					_player_name(
						int(
							result.get(
