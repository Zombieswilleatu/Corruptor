...
static func run(_baseline_rules: RuleConfig) -> Array:
	return [
		...
		_test_humbaba_sigil_commitment(),
		_test_ward_refund(),
...
static func _test_profile() -> Dictionary:
	...
	if (
		...
		return _fail(
			...
			"Measured clocks or enabled systems drifted."
		)
...
static func _test_humbaba_sigil_commitment() -> Dictionary:
	var rules := RuleConfig.lab_v6_5()
	var fixture: Dictionary = _fixture(rules)
	var game = fixture["game"]
	var defender = fixture["p0"]
	var attacker = fixture["p1"]

	defender.lord = "Humbaba"
	defender.hand.clear()
	attacker.hand = [CardData.new("Butcher", 2)]
	...
	if (
		...
		or not bool(defender.ward_turned.get("Lord", false))
		or String(defender.sigils.get("Lord", "")) != "fresh"
	):
		return _fail(
			"unit_lab_humbaba_sigil_commitment",
			"Humbaba's fixed Sigil contribution did not turn an equal Hunt."
		)

	return _pass("unit_lab_humbaba_sigil_commitment")
