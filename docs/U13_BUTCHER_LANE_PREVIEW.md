...
Corrected selector names: **Inspect celebrate** (row three) and **Inspect attack** (row four). Attack frames 4–5 use polygon boundaries to exclude adjacent shield effects without a straight vertical cut through trailing hair; frame 6 uses a tighter rectangle. Existing ground anchors and body scale are preserved. The source PNG and death animation remain unchanged.
