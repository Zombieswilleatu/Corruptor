...
| Repairs | Controller explicitly loops further Repairs and permits the remaining Construct action. | U13 accepts one Castle action in a complete round order. This is a real difference, but the U13 construction document explicitly chose that interface. Reconcile repeated maintenance with the locked-order model; do not silently label it U12 parity or discard accepted construction changes. |
...
The user runner remains **8/8**. Local compatibility runtime is Godot 4.5.1;
Windows 4.7.2 remains the user gate.

## Revised queue

1. Blood Conduit and the Sigil lifecycle, with current U13 Lord interactions.
2. Reconcile repeated Repair with the accepted construction/locked-order model;
   settle provisional opening payment/loadout economy before declaring parity.
3. Waiter spending, Dominion rites, Vacant Throne, Veil and actual victory.
4. Full-match Random-Legal acceptance, then playable UI with the Slaver's actual
