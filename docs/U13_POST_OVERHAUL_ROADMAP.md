> **2026-09-18 Odradek horizon, local verification:** [Common doctrine V7](U13_ODRADEK_HORIZON_2026-09-18.md) compares Reconfiguration saving with immediate casts, accounts for own Guard/attack conflicts, and removes Redirect credit for merely moving an intact crowd. The corrected 34-game development pilot finished 17-17 with zero failures/rejections; False Orders fizzles fell 8 to 2, while both policies cast zero Inversions. A directed test verifies saving at three then actually transferring Guards. The pilot predates `66ea09e`; after integrating that movement fix, CPython passed 120 tests and five games / 88 rounds / 2,220 operations at `78719b9`. Also repairs the Python Gremory reaction to tower kills to match native behavior. Windows CPython/PyPy remains pending; no new native parity or strength/balance conclusion. Next: the bounded Windows check, then behavior review without forcing Inversion frequency.
