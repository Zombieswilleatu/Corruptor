...
The playable U13 board defaults to the original `Chits.png` symbols for ordinary
marchers and animated still sprites for monsters. Two independent buttons below
the battlefield's ACTION heading switch `Units: Chits/Sprites` and
`Monsters: Sprites/Chits`. Preferences persist locally across launches, separately
from match saves. Monster chit mode uses distinct two-letter tokens, with full
names and status on hover. Void still obscures HP in the tooltip.

The clash window always uses the approved still poses and Godot motion from the
animation gallery. Wide field sprites have a proportional width cap; regular
chits use a compact 28-pixel footprint. Sprite feet remain at the recorded world position;
...
The distinct `SUPPLICANT` waiting-state label remains. Chits retain the same
arrival/hit flashes, death ghost, Rout overlay, Paradox slices and HP/Armor data.
...
A monster uses the existing `kind: "marcher"` presentation contract
...
already in turret form starts in the final pose. The implemented monster rules
enforce permanent rooting, movement and combat stats.

Sooge selects the nearest enemy in its lane within 600 fixed-point range. Each
shot records one `MONSTER_BEAM_FIRED` event with that aim and maximum range;
per-victim `MONSTER_ATTACK` records remain the authority for damage. The beam
pierces through the selected enemy to maximum range, including other enemies
and friendly collateral. Current damage is 3 against enemies and 1 against
allies, absorbed by Armor first. The visual is a bright red 0.22-second pulse
with a pale core, eye flash and individual hit flashes. Collateral never redirects
the beam or creates extra rays. It respects lane bounds and works in all display
modes. Final pulses drain before playback finishes; world/reset clears them.

Unknown art falls back to a marker; missing subject art uses its original chit.
Resetting the board clears per-entity presentation state while preserving the
two display preferences. Full frame animation can replace this presentation later.

## Checks

Run `Scripts/Sim/U13MarcherSpritesTestRunner.gd` headlessly for asset mappings,
motion/feedback, input immutability, monster forms, independent display controls, board drawing,
...
September 17 local diagnostics (Godot 4.5.1): 63 sprite/display checks,
135 monster checks and the real board feedback runner pass. Native and Python
monster phases match every event and all 2,400 recorded ticks. Integer-division
warnings remain errors in the diagnostic project.
The Vulture ranged suite passes with the established 32-tick ranged cadence.
Monster checks cover nearest aim, friendly collateral, enemies behind the first
target, maximum range, beam width and one pulse per shot. Godot 4.7.2 remains the game's
