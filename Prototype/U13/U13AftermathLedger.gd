...
static func render(world: Dictionary, events: Array, round_number: int, before: Dictionary = {}) -> String:
	...
	for pid in [0, 1]:
		...
		for entity in world.get("entities", []):
			if entity.get("owner") != pid or entity.get("kind") not in ["lord", "castle"]: continue
			for old in before.get("entities", []):
				if old.id != entity.id: continue
				for key in ["integrity", "threat", "alive", "status", "construction_state"]:
					var now = entity.get("attributes", {}).get(key)
					var was = old.get("attributes", {}).get(key)
					if now != null and was != null and now != was:
						lines.append("• %s · %s: %s → %s" % [name_of(entity), key.capitalize(), str(was), str(now)])
		sections.append("\n".join(lines))
...
static func describe(kind: String, d: Dictionary) -> String:
	match kind:
		...
		"GUARD_DEFEATED": return "Defeated " + name_of(d.get("guard", {"kind": "guard"}))
		"MARCHER_DEFEATED": return "Killed " + name_of(d.get("victim", {"kind": "marcher"}))
		...
		"MARCHER_SPAWNED": return "Raised " + name_of(d) + " · " + str(d.get("attributes", {}).get("lane", ""))
		"SIEGE_RESOLVED": return "Siege: %d Castle damage; %d Guards defeated" % [d.get("damage", 0), d.get("guards_defeated", 0)]
		...
		"CASTLE_REPAIRED", "CASTLE_RESTORED", "CASTLE_ACTIVATED", "CONSTRUCTION_PROGRESS", "LORD_RESUMMONED", "SIGIL_CREATED", "SIGIL_DECAYED", "SIGIL_EXPIRED", "MARCHER_ALLEGIANCE_CHANGED", "FRACTURE_RESOLVED", "KANIFOUS_PRICE_PAID", "KANIFOUS_PRICE_SCHEDULED", "COMBAT_ORDER_FIZZLED", "POWER_RESOLVED", "PROFANE_RESOLVED":
