...
func _ready() -> void:
	...
	for lane in ["Lord", "Castle"]:
		...
		picker.text = "MARCH"
		picker.tooltip_text = "Send the currently eligible staged group in this round's closing march. Recruits born this round remain visible and protected until at least next round. Capacity: 15 per side per lane; oldest eligible reserves march automatically to make room."
...
func bind(state: Dictionary, number: int, modes: Dictionary, editable: bool) -> void:
	...
	for lane in pickers:
		...
		pickers[lane].text = "MARCH · THIS ROUND" if requested else "MARCH"
		pickers[lane].disabled = not editable or requested or not state.lanes[lane].units.any(func(u): return u.owner == 0 and int(u.attributes.staged_round) < number)
		...
		for pid in [0, 1]:
			...
			captions[key].text = "%s · %d/15%s" % ["YOUR STAGING" if pid == 0 else "ENEMY STAGING", units.size(), " · MARCH R%d" % due if due > 0 else ""]
